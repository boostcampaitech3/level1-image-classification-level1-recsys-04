...
def train(data_dir, model_dir, args):
    ...
    model = load_model(save_dir, num_classes,device)
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            mask_preds = torch.argmax(out1, dim=-1)
            gender_preds = torch.argmax(out2, dim=-1)
            age_preds = torch.argmax(out3, dim=-1)

            loss1 = criterion(out1, mask_labels)
            loss2 = criterion(out2, gender_labels)
            loss3 = criterion(out3, age_labels)

            loss = 0.2 * loss1 + 0.4 * loss2 + 0.6 * loss3
