...
class KfoldCutMixDataset(CutMixDataset):
    def __init__(self, data_dir, mean=(0.527, 0.465, 0.435), std=(0.24, 0.242, 0.243), val_ratio=0):
        super().__init__(data_dir, mean, std, val_ratio)

    def set_index_info(self, train_idx, valid_idx):
        for _idx in train_idx:
            self.istrain[_idx] = "train"
        
        for _idx in valid_idx:
            self.istrain[_idx] = "val"
        
        self._set_class_labels(valid_idx= valid_idx)

    def _set_class_labels(self, valid_idx):
        '''
            class_index에 validation_set에 있는 인덱스를 제거해줌
        '''
        for idx, labels in enumerate(self.class_labels):
            labels = [i for i in labels if i not in valid_idx]
            self.class_labels[idx] = labels

    def get_sampler(self, sub_index) :
        '''
            sub_index => train_idx or valid_idx
            train_idx 또는 valid_idx 를 받아 해당 데이터셋의 WeightedRandomSampler 를 반환
        '''
        _multi_class = []
        # -- set _multi_class for sub_index(train or valid)
        for _idx in sub_index:
            _temp = self.encode_multi_class(self.mask_labels[_idx],
                                    self.gender_labels[_idx],
                                    self.age_labels[_idx])
            _multi_class.append(_temp)
